...
def run_scan(dry_run: bool = True) -> dict:
    ...
    for symbol in FALLBACK_WATCHLIST:
        ...
        bezet = open_of_pending(traded, "RVB")
        if bezet >= MAX_OPEN_POSITIONS:
            ...
            try:
                ...
                log_decision(f"⏸️ RVB {signal.direction} {symbol}: signaal @ {signal.trigger_price:.2f} overgeslagen -- {reden}",
                             strategy="RVB", symbol=symbol)
            except Exception:
                pass
            continue
