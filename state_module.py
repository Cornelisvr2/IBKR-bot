...
def add_position(position: dict, path: str = None) -> dict:
    ...
    Args:
        position: dict met minimaal 'symbol', 'direction', 'entry_price',
                  ...
                  Bevat idealiter ook 'oca_group' (zie remove_position).
    """
    state = load_state(path)
...
def remove_position(symbol: str, oca_group: str = None, path: str = None) -> dict:
    """
    ...
    BUGFIX (9 sep 2026): sinds TTS en QFS BEIDE tegelijk op dezelfde
    aandelen kunnen draaien (A/B-vergelijking), kan hetzelfde symbool
    nu WEL degelijk meerdere gelijktijdige, open posities hebben --
    de eerdere aanname dat dit "niet van toepassing" was, klopt niet
    meer. Zonder onderscheid verwijderde dit de EERSTE match op
    symbool, wat de verkeerde (nog open) positie kon treffen als de
    ANDERE strategie's positie op hetzelfde symbool het eerst sloot.

    Als `oca_group` wordt meegegeven (altijd beschikbaar via
    spec.oca_group, en gegarandeerd uniek per symbool+richting+
    strategie+prijs), wordt EXACT die positie verwijderd. Zonder
    oca_group valt dit terug op het oude gedrag (eerste match op
    symbool) -- voor achterwaartse compatibiliteit.
    """
    state = load_state(path)
    ...
    for i, pos in enumerate(positions):
        if pos.get("symbol") != symbol:
            continue
        if oca_group is not None and pos.get("oca_group") != oca_group:
            continue
        removed = positions.pop(i)
        save_state(state, path)
        logger.info(f"Positie verwijderd: {symbol}" + (f" ({oca_group})" if oca_group else ""))
        return state
    logger.warning(f"Geen open positie gevonden voor {symbol} om te verwijderen." + (f" (oca_group={oca_group})" if oca_group else ""))
    return state
